...
const ProcessVideoPage: Component<ProcessVideoPageProps> = (props) => {
    ...
    const [isLoadingTemplates, setIsLoadingTemplates] = createSignal(true);
    ...
    const fetchTemplates = async () => {
        try {
            ...
            if (result.success && result.data) {
                ...
                console.warn('Failed to fetch templates:', result.error);
                // Don't show error for templates, just use default
                setTemplates([]);
            }
        } catch (err) {
    ...
    const handleSubmit = async (e: Event) => {
        ...
        try {
            ...
            const currentSettings = settings();
            const result = await window.electronAPI.createTranscoderJob(formData(), currentSettings);

            if (result.success && result.data) {
                setSuccess(`Job created successfully! Job ID: ${result.data.name.split('/').pop()}`);
                // Reset form
                setFormData({
                    inputUri: '',
                    outputUri: '',
                ...
                setError(result.error || 'Failed to create transcoding job');
            }
        } catch (err) {
            console.error('Error creating job:', err);
            setError('Failed to create transcoding job. Please check your settings.');
        } finally {
