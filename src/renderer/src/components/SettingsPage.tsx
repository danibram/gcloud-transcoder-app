...
import { GoogleCloudSettings, saveSettings, settings } from '../stores/settingsStore';
...
const SettingsPage: Component<SettingsPageProps> = (props) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const testResult = await window.electronAPI.testGoogleCloudConnection(localSettings());
            ...
            if (testResult.success) {
                ...
                setValidationErrors([testResult.error || 'Failed to connect to Google Cloud']);
                setSaveStatus('error');
            }
        } catch (error) {
            setValidationErrors(['Failed to test connection']);
            setSaveStatus('error');
        } finally {
            setIsSaving(false);
        }
