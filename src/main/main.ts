...
async function updateTemplateCLI(templateId: string, template: Partial<JobTemplate>, settings: GoogleCloudSettings): Promise<JobTemplate> {
    // Note: gcloud transcoder templates does NOT have an 'update' command.
    // The only way to "update" a template is to delete it and recreate it.
    try {
        console.log(`Updating template ${templateId} by delete + recreate...`);

        // Step 1: Delete the existing template
        console.log('Step 1: Deleting existing template...');
        await deleteTemplateCLI(templateId, settings);

        // Step 2: Recreate the template with the new configuration
        console.log('Step 2: Creating template with new configuration...');
        ...
        const createdTemplate = await createTemplateCLI(newTemplate, settings);

        console.log('Template updated successfully (via delete + recreate)');
        return createdTemplate;
    } catch (error: any) {
        console.error('CLI Error updating template:', error);
        // Add context to the error
        if (error.message) {
            error.message = `Failed to update template (delete + recreate): ${error.message}`;
        }
        throw error;
    }
}
...
async function createJobCLI(jobData: {
    ...
    try {
        // Note: gcloud transcoder jobs create does NOT accept a job name as positional argument
        // The job ID is auto-generated by Google Cloud
        let command = `gcloud transcoder jobs create --location=${settings.location} --project=${settings.projectId}`;
